# home.git — full deployment git (identity from cosmos options, ssh signing,
# allowed_signers). The portable wrapper variant stays in the hm-wrapper catalog.
{cosmosLib, ...}: let
...
in {
  den.aspects.home.git.homeManager = {
    config,
    lib,
    ...
  }: let
  ...
  in {
    options.cosmos.cli.programs.git = {
      user = mkOption {
        type = str;
        default = "LarsvanDartel";
        description = "User name for git";
      };
      email = mkOption {
        type = str;
        default = "larsvandartel73@gmail.com";
