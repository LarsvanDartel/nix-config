...
in {
  configurations.endeavour = {
    ...
    module = {
      ...
      # hardware-configuration.nix. Generate: nixos-facter -o modules/hosts/endeavour/_hw/endeavour.facter.json
      hardware =
        if builtins.pathExists ./_hw/endeavour.facter.json
        then [
          inputs.nixos-facter-modules.nixosModules.facter
          {facter.reportPath = ./_hw/endeavour.facter.json;}
        ]
    ...
    in {
      ...
      # Host-specific home (the base is wired centrally by the `common` aggregate).
      home-manager.users.nixos.cosmos.system.impermanence.persist.directories = ["dev"];
